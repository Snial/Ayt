...
struct symbol symtab[SYMBOLS] = {  /* values = 01xxxx indicate MRI */
...
/* define symbol */
void define(char sym[SYMLEN], short int val )
{
	int i,j;
...
void putout( short int loc, short int val )
{
	if (lst != NULL) {
		if (listed == 0) {
...
void deflex( int start, int term, int val )
{
	char sym[SYMLEN];
	int from, to;
